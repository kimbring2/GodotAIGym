...
class Basic3DPlatformer(gym.Env):
	def __init__(self, exec_path, env_path, num_actions=5, num_observations=128*128*3, turbo_mode=False, env_id=0):
		self.handle = "environment" + str(env_id)
		...
		self.agent_action_tensor = self.mem.newFloatTensor("agent_action_" + self.handle, 5)
		...
		with open("stdout.txt","wb") as out, open("stderr.txt","wb") as err:
			self.process = subprocess.Popen([exec_path, "--handle", self.handle], stdout=out, stderr=err)
